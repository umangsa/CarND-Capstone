...
class TLClassifier(object):
    def __init__(self):
        ...
        # rospy.init_node('TLClassifier')
        rospack = rospkg.RosPack()
        pkg_path = rospack.get_path('tl_detector')
        self.class_threshold = 0.01

        self.config = tf.ConfigProto()
        # self.config.gpu_options.allow_growth = True
        # self.config.graph_options.optimizer_options.global_jit_level = tf.OptimizerOptions.ON_1
        self.detection_graph = load_graph(os.path.join(pkg_path, "light_classification", "ssd_detection_optimized.pb"), "detection")
